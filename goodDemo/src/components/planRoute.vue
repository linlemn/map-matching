        ...
        methods: {
            ...
            createFavouriteDiv: function(poi) {
                ...
                function addFavourite(e) {
                    for (var i = 0; i < self.pois.length; i++) {
                        //找到收藏点的id
                        if (self.pois[i][0].location.lat == poi.location.lat && self.pois[i][0].location.lng == poi.location.lng) {
                            index = i
                            break
                        }
                    }
                    ...
                    //加入收藏列表
                    $.post(self.urlHeader + '/favourites/path', {
                            // pathId: pathId,
                            desId: index,
                        },
                        function(data) {
            ...
            getRoute: function() {
                ...
                function postDestinations(destinations) {
                    ...
                    function postPreferLeaveTime(identifier) {
                        ...
                        for (var i = 0; i < self.departureTime; i++) {
                            if (self.departureTime[i][0] == identifier) {
                                ...
                                $.post(self.urlHeader + '/path/preferLeaveTime', {
                                        // pathId: pathId,
                                        pathId: 1,
                    ...
                    for (var i = 0; i < destinations.length; i++) {
                        ...
                        $.post(self.urlHeader + '/path/destination', {
                                // pathId: pathId,
                                pathId: 1,
                                desOrder: i + 1,
                            ...
                            switch (this.pois[i][2]) {
                                ...
                                console.log("bus")
                                    AMap.service(["AMap.Transfer"], function() {
                                        ...
                                        //根据起、终点坐标查询公交换乘路线
                                        var start = [self.pois[i][0].location.lng, self.pois[i][0].location.lat]
                                        var end = [self.pois[i + 1][0].location.lng, self.pois[i + 1][0].location.lat]
